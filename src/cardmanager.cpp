...
static bool isEEPROMBlank() {
    if (EEPROM.read(EEPROM_COUNT_ADDR) != 0xFF) {
        return false;
    }
...
static void addDefaultCardDirect() {
    writeUID(0, UID_VALID);

    s_count = 1;
    EEPROM.update(EEPROM_COUNT_ADDR, s_count);

    Serial.println(F("CardManager: card default adaugat in EEPROM"));
}
...
#if ENABLE_DEFAULT_CARD
    if (s_count == 0) {
        Serial.println(F("CardManager: EEPROM gol"));
        addDefaultCardDirect();
    }
#endif
